...
unsigned int btrfs_get_block( unsigned int fd ){
  //returns first block
  return(directory[fd].first_block);
}
unsigned int btrfs_check_file_is_closed( unsigned int fd ){
...
unsigned int btrfs_delete( unsigned int file_descriptor ){
  ...
  if( !btrfs_check_file_is_closed( file_descriptor ) ) return( FALSE );
  //Var assignments
...
unsigned int btrfs_read( unsigned int file_descriptor,
  ...
  if( !btrfs_check_block_in_range ( btrfs_get_block(file_descriptor) )) return( FALSE );
  if( directory[file_descriptor].byte_offset > btrfs_size(file_descriptor)) return(FALSE);
 //Var assignments
  unsigned char b;
  ...
  //find Block
  for (int i = 0; i < (int)offset_block; i++){
...
unsigned int btrfs_write( unsigned int file_descriptor,
                        ...
                        unsigned int byte_count ){
  // Preconditions
  if( !btrfs_check_fd_in_range ( file_descriptor ) ){ 
    
    return( FALSE );
  }
  if( !btrfs_check_file_is_open ( file_descriptor ) ){ 
